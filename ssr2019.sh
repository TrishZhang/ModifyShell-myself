...
#=================================================
#	System Required: CentOS 7/Debian 6+/Ubuntu 14.04+
#   Copy From Teddysun
#   Thanks to a unknown Foreign organization
#=================================================
...
Add_firewalld(){
	if [[ ! -z "${ssr_port}" ]]; then
		firewall-cmd --zone=public --permanent --add-port=${ssr_port}/tcp
		firewall-cmd --zone=public --permanent --add-port=${ssr_port}/udp
	fi
}
Del_firewalld(){
	if [[ ! -z "${port}" ]]; then
		firewall-cmd --zone=public --permanent --remove-port=${ssr_port}/tcp
		firewall-cmd --zone=public --permanent --remove-port=${ssr_port}/udp
	fi
}
Save_firewalld(){
...
Set_config_port(){
	...
	read -e -p "(默认: 60000):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="60000"
...
Install_SSR(){
	...
	echo -e "${Info} 开始设置 firewalld防火墙..."
	Set_firewalld
	echo -e "${Info} 开始添加 firewalld防火墙规则..."
	Add_firewalld
	echo -e "${Info} 开始保存 firewalld防火墙规则..."
	Save_firewalld
...
————————————
  ${Green_font_prefix}4.${Font_color_suffix} 一键封禁 BT/PT/SPAM (firewalld)
  ${Green_font_prefix}5.${Font_color_suffix} 一键解封 BT/PT/SPAM (firewalld)
————————————
